...
from src.scheduled_prompts import add_prompt_for_airflow
...
"""
        ...
        # Normalize client name case (NLU might return wrong case)
        if client_name:
            try:
                await client.ensure_token()
                all_clients = await client_cache.get_or_set("all_clients", client.get_clients)
                for c in all_clients:
                    if isinstance(c, dict) and c.get("fm_client_name", "").lower() == client_name.lower():
                        client_name = c.get("fm_client_name")  # Use correct case from API
                        break
            except Exception:
                pass  # Keep original if lookup fails
        ...
        if pq.intent == "basic_analytics_item" and pq.item:
            ...
            try:
                _generate_and_send_text_report(
                    metric_response, client_name, fleet_name, time_phrase, timezone, time_strings
                )
                add_prompt_for_airflow(clean_text, PROJECT_ROOT)
            except Exception as e:
                logger.warning("Text-based report generation after metric failed: %s", e)
            return metric_response
        else:
            # Fetch once: return summary to user and auto-generate + send report (no separate script)
            summary_text, data, time_strings = await _fetch_analytics_data_and_summary(
                client_name, fleet_name, time_phrase, timezone
            )
            try:
                _generate_and_send_text_report(
                    summary_text, client_name, fleet_name, time_phrase, timezone, time_strings
                )
                add_prompt_for_airflow(clean_text, PROJECT_ROOT)
            except Exception as e:
                logger.warning("Text-based report generation failed: %s", e)
            return summary_text
